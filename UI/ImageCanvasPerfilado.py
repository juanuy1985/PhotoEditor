...
from Processing.Profile import getProfile
from kivy.core.window import Window
import os.path

class ImageCanvasPerfilado(Widget):

    def __init__(self, **args):
        super(ImageCanvasPerfilado, self).__init__(**args)
        self._fileName = ''
    ...
    def process(self):
        ...
        if os.path.isfile(self.fileName) :    
            ...
            processedFile = self.fileName.replace('.bmp', '_processedPerfilado.bmp')
            self.processPerfilado(self.fileName, processedFile)
    ...
    def processPerfilado(self, fileName, processedFileName):
        profiled = getProfile(fileName)
