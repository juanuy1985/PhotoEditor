...
from Processing.Histogram import getHistogram
from kivy.core.window import Window
import os.path

class ImageCanvasHistograma(Widget):

    def __init__(self, **args):
        super(ImageCanvasHistograma, self).__init__(**args)
        self._fileName = ''
    ...
    def process(self):
        ...
        if os.path.isfile(self.fileName) :    
            ...
            processedFile = self.fileName.replace('.bmp', '_processed3.bmp')
            self.processHistograma(self.fileName, processedFile)
    ...
    def processHistograma(self, fileName, processedFileName):
        image = getHistogram(fileName)
        image.save(processedFileName)
